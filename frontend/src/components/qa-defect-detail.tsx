...
export function QADefectDetail({ defectId }: { defectId: string }) {
  ...
  if (error) return <ErrorState title="Cacat QA tidak tersedia" description={error} />;
  if (!defect || checking) return <LoadingState label="Memuat cacat QA" />;

  async function createTicket() { setBusy(true); setError(''); try { const created = await createMaintenanceTicketFromDefect(defect!); setTicket(created); setSuccess('Tiket pemeliharaan dibuat dari cacat QA ini.'); } catch (reason) { setError(qaApiMessage(reason)); } finally { setBusy(false); } }
